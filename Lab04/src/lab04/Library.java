...
public class Library<Type>{
  ...
  public Type lookup(long isbn) {
	for (int i = 0; i < library.size(); i++){
		if (library.get(i).getIsbn() == isbn){
			Type a = library.get(i).getHolder();
			return a;
  ...
  protected class OrderByIsbn implements Comparator<LibraryBook<Type>> {
 	...
 	public int compare( LibraryBook<Type>m, LibraryBook<Type>n) 
 	{
 		return Double.compare(m.getIsbn(), n.getIsbn());
 	}
  }
 ...
 /**
  ...
  public ArrayList<LibraryBook<Type>> getOverdueList(int month, int day,
	  ...
	  ArrayList<LibraryBook<Type>> sortedlist = new ArrayList<LibraryBook<Type>>();
	  for(int i = 0; i < library.size(); i++) {
		  if ( library.get(i).getDueDate().after(inputDate)) {
			  
		    sortedlist.add(library.get(i));
		  }
	  }
	  
	  sortedlist.sort(new OrderByDueDate());
	  return sortedlist;
...
Comparator<LibraryBook<Type>> {
	  ...
	  public int compare(LibraryBook<Type>m, LibraryBook<Type>n)
	  {		if (m.getAuthor()!=n.getAuthor()) {
		  return m.getTitle().compareTo(n.getTitle());
	  }
		  return m.getAuthor().compareTo(n.getAuthor());
	  }
  ...
  protected class OrderByDueDate implements Comparator<LibraryBook<Type>> {
    ...
    public int compare(LibraryBook<Type>m, LibraryBook<Type>n)
    {
    	return m.getDueDate().compareTo(n.getDueDate());
    }
